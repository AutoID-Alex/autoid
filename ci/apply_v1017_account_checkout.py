...
# Version bump must be exact and fail loudly if the v1.0.16 base was not prepared.
s=GRADLE.read_text()
...
# Models for editable account, payment methods and order details.
s=MODELS.read_text()
...
# API v1.0.17 account/order client.
s=API.read_text()
...
# v1.0.17 copy/UX refinements are applied to the known-good v1.0.16 screens.
s=V114.read_text()
replacements={
...
# Stable v1.0.17 entry points. Keeping wrappers separate avoids duplicating private Compose helpers.
