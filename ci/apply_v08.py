...
ROOT=Path('.')
# Restore full v0.8 sources/bridge from compact payload.
payload=(ROOT/'ci/v08_source_payload.txt').read_text().strip()
...
# Bundle the user-provided AutoID icon/logo locally into the APK.
for name,count in [('icon',5),('logo',4)]:
    b64=''.join((ROOT/f'ci/v08_{name}_{i:02d}.txt').read_text().strip() for i in range(count))
    p=ROOT/f'android-v0.1/app/src/main/res/drawable-nodpi/autoid_{name}.png';p.parent.mkdir(parents=True,exist_ok=True);p.write_bytes(base64.b64decode(b64))

# Patch v0.7 root navigation to the v0.8 native screens while preserving proven product-family logic.
p=ROOT/'android-v0.1/app/src/main/java/ro/autoid/app/MainActivity.kt';s=p.read_text()
...
# Version and branding resources.
p=ROOT/'android-v0.1/app/build.gradle.kts';s=p.read_text().replace('versionCode = 6','versionCode = 8').replace('versionName = "0.6.0"','versionName = "0.8.0"');p.write_text(s)
p=ROOT/'android-v0.1/app/src/main/AndroidManifest.xml';s=p.read_text().replace('android:icon="@drawable/ic_autoid"','android:icon="@drawable/autoid_icon"').replace('android:roundIcon="@drawable/ic_autoid"','android:roundIcon="@drawable/autoid_icon"');p.write_text(s)

release='''# AutoID Professional Solutions v0.8.0\n\nNative commerce/UIX release. AI integration is intentionally deferred.\n\n- Bundled local AutoID icon and logo.\n- Home: grouped products only in the 8 requested categories, ordered by summed child `stock_autoid`.\n- Native child-category chips.\n- Functional native header cart navigation.\n- Native checkout with Company, VAT, COD, bank transfer and Stripe placeholder guarded until PaymentSheet is wired.\n- Native registration and improved account UI with Google sign-in entry point.\n- Native Analytics/Marketing privacy preferences.\n- v0.7 grouped parent/child + product_tag relationship logic preserved.\n'''
