...
class CommerceStore(context: Context) {
    ...
    fun cart(): List<CartLine> = readArray("cart").mapNotNull { o ->
    ...
    fun clearCart() = prefs.edit().remove("cart").apply()
    fun cartCount(): Int = cart().sumOf { it.quantity }

    fun wishlistIds(): Set<Long> = prefs.getStringSet("wishlist", emptySet()).orEmpty().mapNotNull { it.toLongOrNull() }.toSet()
